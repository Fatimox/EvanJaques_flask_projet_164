...
"""
    Nom : parents_telephone_afficher
    Auteur : OM 2021.05.01
    Définition d'une "route" /parents_telephone_afficher
...
@app.route("/parents_telephone_afficher/<int:id_genre_sel>", methods=['GET', 'POST'])
def parents_telephone_afficher(id_genre_sel):
    print(" parents_telephone_afficher id_genre_sel ", id_genre_sel)
        ...
        try:
            with DBconnection() as mc_afficher:
                ...
                if id_genre_sel == 0:
                    ...
                    valeur_id_film_selected_dictionnaire = {"value_id_enfants_selected": id_genre_sel}
                ...
                if not data_genres_films_afficher and id_genre_sel == 0:
                    flash("""La table "t_enfants" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_genre_sel > 0:
                    # Si l'utilisateur change l'id_enfants dans l'URL et qu'il ne correspond à aucun film
                    flash(f"Le film {id_genre_sel} demandé n'existe pas !!", "warning")
                else:
                    flash(f"Données enfants et parents affichés !!", "success")

        except Exception as Exception_films_genres_afficher:
            raise ExceptionFilmsGenresAfficher(f"fichier : {Path(__file__).name}  ;  {parents_telephone_afficher.__name__} ;"
                                               f"{Exception_films_genres_afficher}")

    print("parents_telephone_afficher  ", data_genres_films_afficher)
    # Envoie la page "HTML" au serveur.
    return render_template("parents_telephone/parents_telephone_afficher.html", data=data_genres_films_afficher)
...
"""
    ...
    Récupère la liste de tous les parents du film sélectionné par le bouton "MODIFIER" de "parents_telephone_afficher.html"
...
def edit_enfants_sante_selected():
    if request.method == "GET":
        try:
            ...
            # Récupère la valeur de "id_enfants" du formulaire html "parents_telephone_afficher.html"
            # l'utilisateur clique sur le bouton "Modifier" et on récupère la valeur de "id_enfants"
            # grâce à la variable "id_film_genres_edit_html" dans le fichier "parents_telephone_afficher.html"
    ...
    return render_template("parents_telephone/enfants_sante_modifier_tags_dropbox.html",
...
def update_genre_film_selected():
    ...
    return redirect(url_for('parents_telephone_afficher', id_genre_sel=id_film_selected))
