...
"""
    ...
    Paramètres : order_by : ASC : Ascendant, DESC : Descendant
                id_factures_sel = 0 >> tous les parents.
                id_factures_sel = "n" affiche le genre dont l'id est "n"
"""


@app.route("/factures_afficher/<int:id_factures_sel>", methods=['GET', 'POST'])
def factures_afficher(order_by, id_factures_sel):
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_factures_sel == 0:
                    strsql_genres_afficher = """SELECT id_factures, Montant, Date FROM t_factures ORDER BY id_factures ASC"""
                    ...
                    valeur_id_genre_selected_dictionnaire = {"value_id_genre_selected": id_factures_sel}
                    strsql_genres_afficher = """SELECT * FROM t_factures WHERE id_factures = %(value_id_genre_selected)s"""

                    mc_afficher.execute(strsql_genres_afficher, valeur_id_genre_selected_dictionnaire)
                else:
                    strsql_genres_afficher = """SELECT * FROM t_factures"""
...
def genre_update_wtf():
    ...
    try:
        ...
        if form_update.validate_on_submit():
            ...
            return redirect(url_for('factures_afficher', order_by="ASC", id_factures_sel=id_genre_update))
