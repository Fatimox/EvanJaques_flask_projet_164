...
from APP_YOGA_164.factures.gestion_factures_wtf_forms import FormWTFAjouterFactures
from APP_YOGA_164.factures.gestion_factures_wtf_forms import FormWTFDeleteFactures
from APP_YOGA_164.factures.gestion_factures_wtf_forms import FormWTFUpdateFactures
...
"""
    ...
    Remarque :  Dans le champ "montant_facture_update_wtf" du formulaire "factures/factures_update_wtf.html",
                le contrôle de la saisie s'effectue ici en Python.
                On transforme la saisie en minuscules.
                On ne doit pas accepter des valeurs vides, des valeurs avec des chiffres,
                des valeurs avec des caractères qui ne sont pas des lettres.
...
def facture_update_wtf():
    ...
    try:
        ...
        if form_update.validate_on_submit():
            ...
            montant_facture_update = form_update.montant_facture_update_wtf.data
            date_facture_update = form_update.date_facture_update_wtf.data

            valeur_update_dictionnaire = {"value_id_facture": id_facture_update,
                                          "value_montant_facture": montant_facture_update,
                                          "value_date_facture": date_facture_update
                                          }
            print("valeur_update_dictionnaire ", valeur_update_dictionnaire)

            str_sql_update_intitulegenre = """UPDATE t_factures SET Montant = %(value_montant_facture)s,
                                                            Date = %(value_date_facture)s
                                                            WHERE id_factures = %(value_id_facture)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_genre = "SELECT id_factures, Montant, Date FROM t_factures " \
                               "WHERE id_factures = %(value_id_facture)s"
            ...
            form_update.montant_facture_update_wtf.data = data_nom_genre["Montant"]
            form_update.date_facture_update_wtf.data = data_nom_genre["Date"]
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_del.data:
                ...
                str_sql_delete_factures = """DELETE FROM t_factures WHERE id_factures = %(value_id_facture)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT id_factures, Montant, Date FROM t_factures WHERE id_factures = %(value_id_facture)s"""
            ...
            with DBconnection() as mydb_conn:
                ...
                # Opération sur la BD pour récupérer "id_factures" et "intitule_genre" de la "t_factures"
                str_sql_id_genre = "SELECT id_factures, Montant, Date FROM t_factures WHERE id_factures = %(value_id_facture)s"

                mydb_conn.execute(str_sql_id_genre, valeur_select_dictionnaire)
                # Une seule valeur est suffisante "fetchone()",
                # vu qu'il n'y a qu'un seul champ "nom genre" pour l'action DELETE
                data_montant_facture = mydb_conn.fetchone()
                print("data_nom_genre ", data_montant_facture, " type ", type(data_montant_facture), " genre ",
                      data_montant_facture["Montant"])

            # Afficher la valeur sélectionnée dans le champ du formulaire "factures_delete_wtf.html"
            form_delete.montant_facture_delete_wtf.data = data_montant_facture["Montant"]
